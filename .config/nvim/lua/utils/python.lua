...
function M.get_module_path()
	...
	if cn_type == "identifier" then
		...
		if parent_type ~= "function_definition" and parent_type ~= "class_definition" then
			vim.notify("Cursor is not on a function or a class!", vim.log.levels.ERROR, {
        title = "Python Utils"
      })
	...
	while true do
		...
		if curr_node_type == "function_definition" or curr_node_type == "class_definition" then
			...
			local id_text = vim.treesitter.get_node_text(identifier, 0)
...
function M.copy_python_path()
	...
	vim.notify("Module path is copied to the clipboard!", vim.log.levels.INFO, {
    title = "Python Utils",
  })
end
